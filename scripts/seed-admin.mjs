...
if (!url || !name || !email || !password) {
  ...
  console.error('Usage: node scripts/seed-admin.mjs --url <url> --name <name> --email <email> --password <password>');
  process.exit(1);
}
...
try {
  ...
  if (!res.ok) {
    ...
    if (msg.toLowerCase().includes('already') || msg.toLowerCase().includes('exist')) {
      console.warn(`⚠️  User already exists — skipping creation, will still update role.`);
    } else {
...
console.log(`    --command "UPDATE user SET role = 'admin' WHERE email = '${email}';"`);
console.log('');
console.log('━'.repeat(50));
